...
{
    public partial class creativityCompetitionlist : System.Web.UI.Page
    {
        ...
        public static string getAllCreativityCompetition(string regDateFrom, string regDateTo
        ...
        {
            ClassCollection.Model.CreativityCompetitionAdminResult result = new ClassCollection.Model.CreativityCompetitionAdminResult();
            ...
            var us = db.creativityCompetitionTbls.Where(c => c.creativityCompetitionContentTbls.Any()).AsQueryable();
            ...
            if (fillterString != "")
            {
                ...
                {
                    string temp = keyword;
                    predicate = predicate.Or(p => p.title.Contains(keyword) || p.creativityCompetitionContentTbls.Any(c=>c.contentType==0 && c.value.Contains(keyword)));
                }
            ...
            result.creativityCompetition = new List<Model.CreativityCompetitionAdmin>();
            foreach (var item in acList)
            {
                ...
                var tmp = new ClassCollection.Model.CreativityCompetitionAdmin();
                tmp.ID = item.ID;
                ...
                result.creativityCompetition.Add(tmp);
            }
        ...
        [System.Web.Services.WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string SendCreativityCompetitionNotification(long creativityCompetitionID, string text)
        {
            ...
            if (!permission.creativityCompetition.access.edit)
            {
                result.code = 2;
            ...
            creativityCompetition.isSendNotification = true;
            creativityCompetition.notificationText = text;
            db.SubmitChanges();
            new service.pushservice().XsendPush("1", "CreativityCompetitionStartNot", creativityCompetition.ID + "", text, "toall", "false");
            result.code = 0;
        ...
        [System.Web.Services.WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string BlockUnblockCreativityCompetition(long creativityCompetitionID, bool isBlock)
        {
            ClassCollection.Model.Result result = new ClassCollection.Model.Result();
            if (!ClassCollection.Methods.isLogin(HttpContext.Current))
            {
            ...
            if (!permission.creativityCompetition.access.delete)
            {
                result.code = 2;
                result.message = ClassCollection.Message.OPERATION_NO_ACCESS;
                return new JavaScriptSerializer().Serialize(result);
            ...
            {
                creativityCompetition.isBlock = false;
            }
