...
namespace AppinnoNew.admin
{
    ...
    /// </summary>
    public class newsUploader : IHttpHandler, IRequiresSessionState
    {
        public class item
        {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                if (permission.news.access.insert == false)
                {
                    result.code = 2;
                ...
                news.userID = (HttpContext.Current.Session["user"] as userTbl).ID;
                var dateArr = pubdate.Split('/');
                news.publishDate = Persia.Calendar.ConvertToGregorian(int.Parse(dateArr[0]), int.Parse(dateArr[1]), int.Parse(dateArr[2]), Persia.DateType.Persian);
                news.regDate = dt;
                ...
                {
                    var subnews = new newsSubGroupTbl();
                    subnews.newsID = news.ID;
                    subnews.subGroupID = item;

                    db.newsSubGroupTbls.InsertOnSubmit(subnews);
                    db.SubmitChanges();
                }
                ...
                foreach (var item in list)
                {
                    ...
                    {
                        var tbl = new newsContentTbl();
                        tbl.value = context.Request.Form[item];
                        tbl.regDate = dt;
                        tbl.newsID = news.ID;
                        tbl.contentType = 0;
                        db.newsContentTbls.InsertOnSubmit(tbl);
                        db.SubmitChanges();
                    }
                    else if (item.Contains("img_"))
                    {
                        ...
                        string fname = context.Server.MapPath(ClassCollection.Methods.getNewsImagesPath() + imagename);
                        try
                        {
                            file.SaveAs(fname);
                        }
                    ...
                    else if (item.Contains("vid_"))
                    {
                        ...
                        var ext = "." + file.ContentType.Split('/')[1];
                        var imagename = "appinno" + "_" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + "_" + Guid.NewGuid().ToString("N") + ext;
                        string fname = context.Server.MapPath(ClassCollection.Methods.getNewsVideosPath() + imagename);
                        try
                        {
                            file.SaveAs(fname);
                        }
               ...
               // new service.pushservice().XsendPush("1", "news", news.ID + "", news.title, "toall", "false");
                result.code = 0;
            ...
            if (context.Request.QueryString["mode"] == "e")
            {
                try
                {
                    long s = long.Parse(context.Request.Form["newsID"].ToString());
                }
                catch
                {

                }
                if (permission.news.access.edit == false)
                {
                    result.code = 2;
                    result.message = ClassCollection.Message.OPERATION_NO_ACCESS;
                    context.Response.Write(new JavaScriptSerializer().Serialize(result));
                ...
                long newsID = long.Parse(context.Request.Form["newsID"].ToString());
                ...
                var deletingPar = db.newsContentTbls.Where(c => c.newsID == newsID && c.contentType == 0);
                foreach (var item in deletingPar)
                {
                    if (!old_Par_Item.Contains(item.ID))
                    {
                        db.newsContentTbls.DeleteOnSubmit(item);
                    }
                }
                var deletingImg = db.newsContentTbls.Where(c => c.newsID == newsID && c.contentType == 1);
                foreach (var item in deletingImg)
                {
                    if (!old_image_Item.Contains(item.ID))
                    {
                        ...
                        if (System.IO.File.Exists(context.Server.MapPath(ClassCollection.Methods.getNewsImagesPath() + img)))
                        {
                            System.IO.File.Delete(context.Server.MapPath(ClassCollection.Methods.getNewsImagesPath() + img));
                        }
                        db.newsContentTbls.DeleteOnSubmit(item);
                    }
                }
                var deletingVideo = db.newsContentTbls.Where(c => c.newsID == newsID && c.contentType == 2);
                foreach (var item in deletingVideo)
                {
                    if (!old_video_Item.Contains(item.ID))
                    {
                        ...
                        if (System.IO.File.Exists(context.Server.MapPath(ClassCollection.Methods.getNewsVideosPath() + vid)))
                        {
                            System.IO.File.Delete(context.Server.MapPath(ClassCollection.Methods.getNewsVideosPath() + vid));
                        }
                        db.newsContentTbls.DeleteOnSubmit(item);
                    }
                }
                ...
                db.newsTagTbls.DeleteAllOnSubmit(db.newsTagTbls.Where(c => c.newsID == newsID));
                db.SubmitChanges();
                ...
                db.newsSubGroupTbls.DeleteAllOnSubmit(db.newsSubGroupTbls.Where(c => c.newsID == newsID));
                db.SubmitChanges();
                ...
                foreach (var item in list)
                {
                    if (item.Contains("lpar_") )
                    {
                        var tbl = db.newsContentTbls.Single(c => c.ID == long.Parse(context.Request.Form[item]));
                ...
                if(news.isBlock==false && news.isPublished==false)
                {
                    news.isPublished = true;
                    db.SubmitChanges();
                    if(news.newsSubGroupTbls.Any()==false)
                    new service.pushservice().XsendPush("1", "news", news.ID + "", news.title, "toall", "false");
