...
namespace AppinnoNew.admin
{
    public partial class newslist : System.Web.UI.Page
    {
        ...
        public static string getAllNews(string regDateFrom, string regDateTo
            , string pubDateFrom, string pubDateTo, string newsstate
            , string fillterString, long subgroupID, string orderType, 
            string sortby, int pageIndex)
        {
            ClassCollection.Model.NewsAdminResult result = new ClassCollection.Model.NewsAdminResult();
            ...
            var us = db.newsTbls.Where(c => c.newsContentTbls.Any()).AsQueryable();
            ...
            if (subgroupID == -1)
            {
                us = us.Where(c => c.newsSubGroupTbls.Any() == false);
            }
            else if (subgroupID == -2)
            {
                us = us.Where(c => c.newsSubGroupTbls.Any());
            }
            else if (subgroupID > 0)
            {
                ...
                us = us.Where(c => c.newsSubGroupTbls.Any(p => p.subGroupID == subgroupID));
            }
            ...
            if (fillterString != "")
            {
                ...
                {
                    string temp = keyword;
                    predicate = predicate.Or(p => p.title.Contains(keyword) || p.newsContentTbls.Any(c=>c.contentType==0 && c.value.Contains(keyword)));
                }
            ...
            result.news = new List<Model.NewsAdmin>();
            foreach (var item in acList)
            {
                var tmp = new ClassCollection.Model.NewsAdmin();
        ...
        [System.Web.Services.WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string deleteNews(long newsID)
        {
            ...
            if (!permission.news.access.delete)
            {
                result.code = 2;
            ...
            db.newsTagTbls.DeleteAllOnSubmit(news.newsTagTbls);
            db.SubmitChanges();

            var contents = db.newsContentTbls.Where(c => c.newsID == news.ID);
            ...
            foreach (var item in contents)
            {
                ...
                if (item.contentType == 1)//image
                {
                    ...
                    if (System.IO.File.Exists(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getNewsImagesPath() + image)))
                    {
                        try
                        {
                            System.IO.File.Delete(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getNewsImagesPath() + image));
                        }
                        catch
                        {
                            result.code = 3;
                ...
                else if (item.contentType == 2)//video
                {
                    ...
                    if (System.IO.File.Exists(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getNewsVideosPath() + video)))
                    {
                        try
                        {
                            System.IO.File.Delete(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getNewsVideosPath() + video));
                        }
                        catch
                        {
                            result.code = 4;
            ...
            db.newsContentTbls.DeleteAllOnSubmit(contents);
            db.SubmitChanges();

            db.newsLikeTbls.DeleteAllOnSubmit(db.newsLikeTbls.Where(c => c.newsID == news.ID));
            db.SubmitChanges();

            db.newsTbls.DeleteOnSubmit(news);
            db.SubmitChanges();
        ...
        [System.Web.Services.WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string deleteAllNews()
        {
            ClassCollection.Model.Result result = new ClassCollection.Model.Result();
            ...
            if (!permission.news.access.delete || !permission.news.access.showlist)
            {
                result.code = 2;
                result.message = ClassCollection.Message.OPERATION_NO_ACCESS;
                return new JavaScriptSerializer().Serialize(result);
