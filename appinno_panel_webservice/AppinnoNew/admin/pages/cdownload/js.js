...
function getAllComment() {
    ...
    var result = cdownload.f1(date, dateto, userstate, filter, order, sortby, PageIndex);
    ...
    $.each(result.comment, function (index, el) {
        ...
        var html =
                    ...
                    + '<div class="clear" />'
                    + '<a target="_blank" href="/admin/edownload.aspx?id=' + el.newsID + '"><div class="link-icon" /><div class="link-text">' + el.newsTitle + '</div></a>'
                    + '</div>');
...
function setup_delete_comment(item) {
    ...
    $(deleteBtn).click(function () {
        if (confirm("آیا از حذف این نظر اطمینان دارید؟")) {
            ...
            var result = cdownload.f4(itemid);
            if (result.code != 0) {
                Siml.messages.add({ text: result.message, type: "error" });
                return;
            }
...
function setup_edit_comment(item) {
    ...
    $(edit).click(function () {
        ...
        {
            var result = cdownload.f2(itemID,true);
            if (result.code == 0) {
                Siml.messages.add({ type: "success", text: result.message });
                getAllComment();
            }
...
function setup_block_comment(item) {
    ...
    $(edit).click(function () {
        ...
        if (confirm("آیا از نمایش نظر اطمینان دارید؟")) {
            var result = cdownload.f2(itemID, false);
            if (result.code == 0) {
                Siml.messages.add({ type: "success", text: result.message });
                getAllComment();
            }
