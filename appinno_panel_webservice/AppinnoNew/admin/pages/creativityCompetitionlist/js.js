...
function getAllItem() {
    ...
    var result = creativityCompetitionlist.f1(date, dateto, startdate, startdateto, enddate, enddateto, itemstate, itemstatus, filter, order, sortby, PageIndex);
    ...
    $.each(result.creativityCompetition, function (index, el) {
        //var block = el.isBlock ? '<a itemid="' + el.ID + '" class="block" title="مسابقه مسدود شده است"></a>' : "";
        ...
        if(el.status == "ثبت شده")
            var html =
                    ...
                    + '<div class="tools">'
                    + '<a  href="ecreativityCompetition.aspx?id=' + el.ID + '" class="edit" title="ویرایش"></a>'
                    + block
                    + '</div>'
                    + '<a href="answerlist.aspx?id=' + el.ID + '" class="ptitle" style="text-decoration: none;color: black;">' + el.title + '</a>'
            ...
            var html =
                    $('<div class="item" itemid="' + el.ID + '">'
                    + '<div class="tools">'
                    + '<a  href="ecreativityCompetition.aspx?id=' + el.ID + '& statu=e" class="edit" title="ویرایش"></a>'
                    + block
...
function setup_insert_para() {
    ...
    $("#btn-save-insert-para-window").click(function () {
        ...
        var result = creativityCompetitionlist.f6(id, text);
...
function setup_block_item(item) {
    ...
    $(blockBtn).click(function () {
        if (confirm("آیا از خارج نمودن این مسابقه از حالت مسدود اطمینان دارید؟")) {
            ...
            var result = creativityCompetitionlist.f7(itemid, false);
            if (result.code != 0) {
                Siml.messages.add({ text: result.message, type: "error" });
                return;
            }
...
function setup_unblock_item(item) {
    ...
    $(blockBtn).click(function () {
        if (confirm("آیا از مسدود نمودن این مسابقه اطمینان دارید؟")) {
            var itemid = $(this).attr('itemid');
            var result = creativityCompetitionlist.f7(itemid, true);
            if (result.code != 0) {
                Siml.messages.add({ text: result.message, type: "error" });
                return;
            }
