...
function check_queryString() {
    ...
    if (itemID == undefined || isNaN(int)) {
        ...
        setTimeout(function myfunction() {
            Siml.page.url.goto('answerlist.aspx');
        }, 2500);

    }
    else {

        var result = eanswer.f4(itemID);
...
function loadData() {
    ...
    $("#pagetitle").attr("href", "answerlist.aspx?id=" + currentItem.creativityCompetitionID);
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        form.append("answerID", currentItem.ID);
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    ...
                    setTimeout(function () { window.location.href = "answerlist.aspx?id=" + Siml.page.url.getParam("p") }, 1500);
                }
                else {
