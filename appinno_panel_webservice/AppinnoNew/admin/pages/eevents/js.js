...
function check_queryString() {
    ...
    if (itemID == undefined || isNaN(int)) {
        ...
        setTimeout(function myfunction() {
            Siml.page.url.goto('eventslist.aspx');
        }, 2500);

    }
    else {

        var result = eevents.f4(itemID);
        ...
        if (result.result.code != 0) {
            ...
            currentItem = result.events;
        }
    }
}
function loadData() {
    $("#main-title").val(currentItem.title);
    $('#date1').val(currentItem.fromDate);
    $('#date2').val(currentItem.toDate);
    var list = $('#items');
    $(list).html('');
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        if (title.val() == "") {
            Siml.messages.add({ text: "عنوان رویداد را وارد کنید", type: "error" });
            return;
        }
        if (date.val() == "") {
            Siml.messages.add({ text: "تاریخ شروع رویداد را وارد کنید", type: "error" });
            return;
        }
        if (date2.val() == "") {
            Siml.messages.add({ text: "تاریخ پایان رویداد را وارد کنید", type: "error" });
            return;
        }
        ...
        form.append("eventsID", currentItem.ID);
        form.append("title", title.val());
        form.append("fromdate", date.val());
        form.append("todate", date2.val());
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "رویداد با موفقیت ویرایش شد" });
                    setTimeout(function () { window.location.href = "eventslist.aspx"; }, 1500);
                }
                else {
