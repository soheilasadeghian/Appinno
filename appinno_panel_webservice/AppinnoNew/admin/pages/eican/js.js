...
function check_queryString() {
    ...
    if (itemID == undefined || isNaN(int)) {
        ...
        setTimeout(function myfunction() {
            Siml.page.url.goto('icanlist.aspx');
        }, 2500);

    }
    else {

        var result = eican.f2(itemID);
...
function loadData() {
    ...
    $.each(currentItem.contents, function (index, el) {
        ...
        if (el.type == 0) {
            ...
            record = $(
                '<div id="lf_' + el.ID + '" class="item file" >' +
                '<img  class="file"/>' +
                '<span class="title">' + 'حجم فایل : ' + el.fileSize + ' کیلوبایت' + '</br>' + 'نوع فایل : ' + el.fileType + '</span>' +
                '<input type="button" class="delete" title="حذف" value="x" />' +
                 //contentedit-start
                '<input type="button" class="up" title="بالا" value="▲" />' +
                '<input type="button" class="down" title="پایین" value="▼" />' +
                '</div>');
            setup_delete_file_item(record);
            setup_arrange_item(record);
            //contentedit-end
        }
        $(list).append(record);
...
function setup_delete_file_item(item) {

    $(item).find(".delete").click(function () {
        var id = $(this).parent().attr('id').replace('lf_', "").replace('f_', "");
        $("#u_" + id).remove();
        $(item).remove();
    });
}
...
/// insert file
function setup_insert_file() {
    file_initOnChange();
}
function file_initOnChange() {
    document.getElementById('file_file').addEventListener('change', null, false);
    document.getElementById('file_file').addEventListener('change', function (e) {
        ...
        if (F && F[0]) for (var i = 0; i < F.length; i++) {
            ...
            var pfi = $("#file_file").parent();
            $("#file_file").attr('id', id = "u_" + ID);
            pfi.append('<input class="file_file" type="file" style="display: none" id="file_file" />');
            $(uploaded).append($("#file_file"));

            var list = $('#items');
            file_initOnChange();
            $(list).append(html);
            setup_delete_file_item(html);
            setup_arrange_item(html);
        }
    }, false);
}
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        if (title.val() == "") {
            Siml.messages.add({ text: "عنوان اصلی توانایی را وارد کنید", type: "error" });
            return;
        }
        if (date.val() == "") {
            Siml.messages.add({ text: "تاریخ انتشار توانایی را وارد کنید", type: "error" });
            return;
        }
        ...
        form.append("icanID", currentItem.ID);
        ...
        $.each(its, function (index, el) {
            ...
            if ($(el).hasClass("file") && $(el).attr('id').indexOf('lf_') < 0) {
                var id = "u_" + $(el).attr('id').replace('f_', '');
                var file = document.getElementById(id).files[0];
                form.append("file_" + index, file);
            }
            else if ($(el).hasClass("file") && $(el).attr('id').indexOf('lf_') >= 0) {
                var id = $(el).attr('id').replace('lf_', '');
                form.append("lfile_" + index, id);
            }
        ...
        $("#bar").on("onFinished", function (event, result) {
            $("#upload-ican-window").css('display', 'none');
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "توانایی با موفقیت ویرایش شد" });
                    setTimeout(function () { window.location.href = "icanlist.aspx"; }, 1500);
                }
                else {
        ...
        $("#upload-ican-window").css('display', 'block');
        eican.f1(form, $("#progress"), $("#bar"));
    });
