...
function check_queryString() {
    ...
    if (itemID == undefined || isNaN(int)) {
        ...
        setTimeout(function myfunction() {
            Siml.page.url.goto('iolist.aspx');
        }, 2500);

    }
    else {

        var result = eio.f4(itemID);
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        if (title.val() == "") {
            Siml.messages.add({ text: "نام سازمان را وارد کنید", type: "error" });
            return;
        }
        if (date.val() == "") {
            Siml.messages.add({ text: "تاریخ انتشار سازمان را وارد کنید", type: "error" });
            return;
        }
        ...
        form.append("ioID", currentItem.ID);
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "سازمان با موفقیت ویرایش شد" });
                    setTimeout(function () { window.location.href = "iolist.aspx"; }, 1500);
                }
                else {
