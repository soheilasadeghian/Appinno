...
function check_queryString() {
    ...
    if (itemID == undefined || isNaN(int)) {
        ...
        setTimeout(function myfunction() {
            Siml.page.url.goto('newslist.aspx');
        }, 2500);

    }
    else {

        var result = enews.f4(itemID);
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        form.append("newsID", currentItem.ID);
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "خبر با موفقیت ویرایش شد" });
                    setTimeout(function () { window.location.href = "newslist.aspx"; }, 1500);
                }
                else {
