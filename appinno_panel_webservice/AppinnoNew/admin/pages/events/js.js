...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        if (title.val() == "") {
            Siml.messages.add({ text: "عنوان اصلی رویداد را وارد کنید", type: "error" });
            return;
        }
        if (date.val() == "") {
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "رویداد با موفقیت درج شد" });
                    setTimeout(function () { window.location.href = "events.aspx"; }, 1500);
                }
                else {
