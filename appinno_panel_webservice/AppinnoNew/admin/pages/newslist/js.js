...
function search() {
    ...
    item.click(function () {
        ...
        if ($("#fchk_allitem").is(':checked')) {
            $("#lt").html("لیست اخبار گروه بندی شده");
            $("#fgroup_panel").slideUp(100);
        }

        if ($("#fchk_toGroup").is(':checked')) {
            $("#lt").html("لیست اخباری که در گروه [ " + $("#group").find('.selected').html() + " ] و زیرگروه  [ " + $("#subgroup").find('.selected').html() + " ] قرار دارند");
...
function getAllItem() {
    ...
    var result = newslist.f1(date, dateto, pubdate, pubdateto, itemstate, filter, subgroupID, order, sortby, PageIndex);
    ...
    var list = $("#list");
    $(list).html('');
    $.each(result.news, function (index, el) {
        var block = el.isBlock ? '<a itemid="' + el.ID + '" class="block" title="خبر مسدود شده است"></a>' : "";
        var html =
                    $('<div class="item" itemid="' + el.ID + '">'
                    + '<div class="tools">'
                    + '<a itemid="' + el.ID + '" class="delete"></a>'
                    + '<a  href="enews.aspx?id=' + el.ID + '" class="edit" title="ویرایش"></a>'
                    + block
                    + '</div>'
    ...
    $('#deleteall-btn').click(function () {
        ...
        if (confirm('آیا از حذف تمام اخبار اطمینان دارید؟')) {
            var result = newslist.f5();
            if (result.code != 0) {
...
function getAllSubGroupForCombo(groupId) {
    ...
    var result = newslist.f3(groupId);
    if (result.result.code != 0) {
        Siml.messages.add({ text: result.result.message, type: "error" });
        return;
    }
