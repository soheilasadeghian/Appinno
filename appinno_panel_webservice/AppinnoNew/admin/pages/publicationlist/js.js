...
function search() {
    ...
    item.click(function () {
        ...
        if ($("#fchk_allitem").is(':checked')) {
            $("#lt").html("لیست انتشارات گروه بندی شده");
            $("#fgroup_panel").slideUp(100);
        }

        if ($("#fchk_toGroup").is(':checked')) {
            $("#lt").html("لیست انتشاراتی که در گروه [ " + $("#group").find('.selected').html() + " ] و زیرگروه  [ " + $("#subgroup").find('.selected').html() + " ] قرار دارند");
...
function getAllItem() {
    ...
    var result = publicationlist.f1(date, dateto, pubdate, pubdateto, itemstate, filter, subgroupID, order, sortby, PageIndex);
    ...
    var list = $("#list");
    $(list).html('');
    $.each(result.publication, function (index, el) {
        var block = el.isBlock ? '<a itemid="' + el.ID + '" class="block" title="انتشارات مسدود شده است"></a>' : "";
        var html =
                    $('<div class="item" itemid="' + el.ID + '">'
                    + '<div class="tools">'
                    + '<a itemid="' + el.ID + '" class="delete"></a>'
                    + '<a  href="epublication.aspx?id=' + el.ID + '" class="edit" title="ویرایش"></a>'
                    + block
                    + '</div>'
    ...
    $('#deleteall-btn').click(function () {
        ...
        if (confirm('آیا از حذف تمام انتشارات اطمینان دارید؟')) {
            var result = publicationlist.f5();
            if (result.code != 0) {
...
function getAllSubGroupForCombo(groupId) {
    ...
    var result = publicationlist.f3(groupId);
    if (result.result.code != 0) {
        Siml.messages.add({ text: result.result.message, type: "error" });
        return;
    }
...
function setup_delete_item(item) {
    ...
    $(deleteBtn).click(function () {
        if (confirm("آیا از حذف این انتشارات اطمینان دارید؟")) {
            var itemid = $(this).attr('itemid');
            var result = publicationlist.f4(itemid);
            if (result.code != 0) {
                Siml.messages.add({ text: result.message, type: "error" });
                return;
            }
