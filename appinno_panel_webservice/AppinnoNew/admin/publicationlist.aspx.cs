    ...
    public partial class publicationlist : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (ClassCollection.Methods.isLogin(Context))
            {
                ...
                if (!permission.publication.access.showlist)
                {
        ...
        public static string getAllpublication(string regDateFrom, string regDateTo
            , string pubDateFrom, string pubDateTo, string publicationstate
            , string fillterString, long subgroupID, string orderType, 
            string sortby, int pageIndex)
        {
            ClassCollection.Model.publicationAdminResult result = new ClassCollection.Model.publicationAdminResult();
            ...
            var us = db.publicationTbls.Where(c => c.publicationContentTbls.Any()).AsQueryable();
            ...
            if (subgroupID == -1)
            {
                us = us.Where(c => c.publicationSubGroupTbls.Any() == false);
            }
            else if (subgroupID == -2)
            {
                us = us.Where(c => c.publicationSubGroupTbls.Any());
            }
            else if (subgroupID > 0)
            {
                ...
                us = us.Where(c => c.publicationSubGroupTbls.Any(p => p.subGroupID == subgroupID));
            }

            if (publicationstate == "block")
            {
                us = us.Where(c => c.isBlock == true);
            }
            else if (publicationstate == "unblock")
            {
            ...
            if (fillterString != "")
            {
                ...
                {
                    string temp = keyword;
                    predicate = predicate.Or(p => p.title.Contains(keyword) || p.publicationContentTbls.Any(c=>c.contentType==0 && c.value.Contains(keyword)));
                }
            ...
            result.publication = new List<Model.publicationAdmin>();
            foreach (var item in acList)
            {
                var tmp = new ClassCollection.Model.publicationAdmin();
        ...
        [System.Web.Services.WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string deletepublication(long publicationID)
        {
            ...
            if (!permission.publication.access.delete)
            {
                result.code = 2;
            ...
            var contents = db.publicationContentTbls.Where(c => c.publicationsID == publication.ID);
            ...
            foreach (var item in contents)
            {
                ...
                if (item.contentType == 1)//image
                {
                    ...
                    if (System.IO.File.Exists(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getpublicationImagesPath() + image)))
                    {
                        try
                        {
                            System.IO.File.Delete(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getpublicationImagesPath() + image));
                        }
                        catch
                        {
                            result.code = 3;
                ...
                else if (item.contentType == 2)//video
                {
                    ...
                    if (System.IO.File.Exists(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getpublicationVideosPath() + video)))
                    {
                        try
                        {
                            System.IO.File.Delete(HttpContext.Current.Server.MapPath(ClassCollection.Methods.getpublicationVideosPath() + video));
                        }
                        catch
                        {
                            result.code = 4;
            ...
            db.publicationContentTbls.DeleteAllOnSubmit(contents);
            db.SubmitChanges();

            db.publicationLikeTbls.DeleteAllOnSubmit(db.publicationLikeTbls.Where(c => c.publicationsID == publication.ID));
            db.SubmitChanges();

            db.publicationTagTbls.DeleteAllOnSubmit(publication.publicationTagTbls);
            db.SubmitChanges();

            db.publicationTbls.DeleteOnSubmit(publication);
            db.SubmitChanges();
        ...
        [System.Web.Services.WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string deleteAllpublication()
        {
            ClassCollection.Model.Result result = new ClassCollection.Model.Result();
            ...
            if (!permission.publication.access.delete || !permission.publication.access.showlist)
            {
                result.code = 2;
                result.message = ClassCollection.Message.OPERATION_NO_ACCESS;
                return new JavaScriptSerializer().Serialize(result);
            ...
            var publication = db.publicationTbls;
