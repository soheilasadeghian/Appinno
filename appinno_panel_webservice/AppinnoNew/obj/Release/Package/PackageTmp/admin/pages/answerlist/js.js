...
function getAllItem(ID) {
    ...
    var result = answerlist.f1(date, dateto, itemstate, filter, order, sortby, PageIndex, ID, justWinner);
    ...
    if (result.winnerID != 0)
    {
        ...
        link.setAttribute("href", "eanswer.aspx?id=" + result.winnerID + "&p=" + ID);
    }
    $.each(result.answer, function (index, el) {
        ...
        var html =
                    ...
                    + '<a  href="eanswer.aspx?id=' + el.ID + '&p=' + ID + '" class="edit" title="ویرایش"></a>'
                    + block
                    + '</div>'
