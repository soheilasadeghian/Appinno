...
function getAllComment() {
    ...
    var result = cio.f1(date, dateto, userstate, filter, order, sortby, PageIndex);
    ...
    $.each(result.comment, function (index, el) {
        ...
        var html =
                    ...
                    + '<div class="clear" />'
                    + '<a target="_blank" href="/admin/eio.aspx?id=' + el.newsID + '"><div class="link-icon" /><div class="link-text">' + el.newsTitle + '</div></a>'
                    + '</div>');
...
function setup_block_comment(item) {
    ...
    $(edit).click(function () {
        ...
        if (confirm("آیا از نمایش نظر اطمینان دارید؟")) {
            var result = cio.f2(itemID, false);
            if (result.code == 0) {
                Siml.messages.add({ type: "success", text: result.message });
                getAllComment();
            }
