...
function check_queryString() {
    ...
    if (itemID == undefined || isNaN(int)) {
        ...
        setTimeout(function myfunction() {
            Siml.page.url.goto('creativityCompetitionlist.aspx');
        }, 2500);

    }
    else {

        var result = ecreativityCompetition.f4(itemID);
        ...
        if (result.result.code != 0) {
            ...
            currentItem = result.creativityCompetition;
        }
    }
}
function loadData() {
    ...
    if (currentItem.canEdit==false) {
        ...
        $(".tools").remove(); $("#btn-save-all").remove();
        $(".sub-right").remove();
    }
...
//tag
function setup_tag() {
    var result = ecreativityCompetition.f5();
    if (result.result.code != 0) {
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        form.append("creativityCompetitionID", currentItem.ID);
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    ...
                    setTimeout(function () { window.location.href = "creativityCompetitionlist.aspx"; }, 1500);
                }
                else {
        ...
        ecreativityCompetition.f1(form, $("#progress"), $("#bar"));
    });
