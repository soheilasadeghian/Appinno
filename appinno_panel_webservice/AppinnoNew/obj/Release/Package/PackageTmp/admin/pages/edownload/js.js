...
function check_queryString() {
    ...
    if (itemID == undefined || isNaN(int)) {
        ...
        setTimeout(function myfunction() {
            Siml.page.url.goto('downloadlist.aspx');
        }, 2500);

    }
    else {

        var result = edownload.f4(itemID);
...
function loadData() {
    ...
    $.each(currentItem.contents, function (index, el) {
        ...
        if (el.type == 0) {
            ...
            record = $(
                '<div id="lv_' + el.ID + '" class="item vid" >'+
                '<img  class="video"/>'+
    ...
    document.getElementById('file_video').addEventListener('change', function (e) {
        ...
        if (F && F[0]) for (var i = 0; i < F.length; i++) {
            ...
            var html = $(
                ...
                + '<img  class="video"/>'
                + '<span class="title">' + 'حجم فایل : ' + size + ' کیلوبایت' + '</br>' + 'نوع فایل : ' + ext + '</span>'
                + '<input type="button" class="delete" title="حذف" value="x" />'
                //contentedit-start
                + '<input type="button" class="up" title="بالا" value="▲" />'
                + '<input type="button" class="down" title="پایین" value="▼" />'
...
function getAllSubGroupForCombo(groupId) {
    var result = edownload.f3(groupId);
    if (result.result.code != 0) {
        Siml.messages.add({ text: result.result.message, type: "error" });
        return;
    }
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        if (title.val() == "") {
            Siml.messages.add({ text: "عنوان اصلی دانلود را وارد کنید", type: "error" });
            return;
        }
        if (date.val() == "") {
            Siml.messages.add({ text: "تاریخ انتشار دانلود را وارد کنید", type: "error" });
            return;
        }
        ...
        form.append("downloadID", currentItem.ID);
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "دانلود با موفقیت ویرایش شد" });
                    setTimeout(function () { window.location.href = "downloadlist.aspx"; }, 1500);
                }
                else {
        ...
        edownload.f1(form, $("#progress"), $("#bar"));
    });
