...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "سازمان با موفقیت درج شد" });
                    setTimeout(function () { window.location.href = "io.aspx"; }, 1500);
                }
                else {
