...
function getAllSubGroupForCombo(groupId) {
    var result = publication.f3(groupId);
    if (result.result.code != 0) {
        Siml.messages.add({ text: result.result.message, type: "error" });
        return;
    }
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "انتشارات با موفقیت درج شد" });
                    setTimeout(function () { window.location.href = "publication.aspx"; }, 1500);
                }
                else {
        ...
        publication.f1(form, $("#progress"), $("#bar"));
    });
