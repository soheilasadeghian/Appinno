...
$(function () {
    ID = 0;
    access_permission();
    Tipped.create('.hint,.tools .button,input.upload , input.save');
...
function setup_save_all() {
    $("#btn-save-all").click(function () {
        ...
        $("#bar").on("onFinished", function (event, result) {
            ...
            try {
                if (result.code == 0) {
                    Siml.messages.add({ type: "success", text: "نمودار با موفقیت درج شد" });
                    setTimeout(function () { window.location.href = "report.aspx"; }, 1500);
                }
                else {
