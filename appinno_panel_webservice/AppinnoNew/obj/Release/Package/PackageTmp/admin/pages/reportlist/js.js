...
function search() {
    ...
    item.click(function () {
        ...
        if ($("#fchk_nogrp").is(':checked')) {
            $("#lt").html("لیست نمودارهای عمومی");
            $("#fgroup_panel").slideUp(100);
        }
        if ($("#fchk_allitem").is(':checked')) {
            $("#lt").html("لیست نمودارهای گروه بندی شده");
            $("#fgroup_panel").slideUp(100);
        }

        if ($("#fchk_toGroup").is(':checked')) {
            $("#lt").html("لیست نمودارهایی که در گروه [ " + $("#group").find('.selected').html() + " ] و زیرگروه  [ " + $("#subgroup").find('.selected').html() + " ] قرار دارند");
...
function getAllItem() {
    ...
    var result = reportlist.f1(date, dateto, pubdate, pubdateto, itemstate, filter, subgroupID, order, sortby, PageIndex);
    ...
    var list = $("#list");
    $(list).html('');
    $.each(result.report, function (index, el) {
        var block = el.isBlock ? '<a itemid="' + el.ID + '" class="block" title="نمودار مسدود شده است"></a>' : "";
        var html =
                    $('<div class="item" itemid="' + el.ID + '">'
                    + '<div class="tools">'
                    + '<a itemid="' + el.ID + '" class="delete"></a>'
                    + '<a  href="ereport.aspx?id=' + el.ID + '" class="edit" title="ویرایش"></a>'
                    + block
                    + '</div>'
    ...
    $('#deleteall-btn').click(function () {
        ...
        if (confirm('آیا از حذف تمام نمودارهای اطمینان دارید؟')) {
            var result = reportlist.f5();
            if (result.code != 0) {
...
function getAllGroupForCombo() {
    ...
    $("#group").on("onChanged", function () {
        ...
        $("#lt").html("لیست نمودارهای که در گروه [ " + $("#group").find('.selected').html() + " ] و زیرگروه  [ " + $("#subgroup").find('.selected').html() + " ] قرار دارند");
        getAllSubGroupForCombo(groupId);
    });
...
function getAllSubGroupForCombo(groupId) {
    ...
    var result = reportlist.f3(groupId);
    if (result.result.code != 0) {
        Siml.messages.add({ text: result.result.message, type: "error" });
        return;
    }
...
function setup_delete_item(item) {
    ...
    $(deleteBtn).click(function () {
        if (confirm("آیا از حذف این نمودار اطمینان دارید؟")) {
            var itemid = $(this).attr('itemid');
            var result = reportlist.f4(itemid);
            if (result.code != 0) {
                Siml.messages.add({ text: result.message, type: "error" });
                return;
            }
