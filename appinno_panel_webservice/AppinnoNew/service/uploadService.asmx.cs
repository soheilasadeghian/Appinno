...
namespace AppinnoNew.service
{
    ...
    public class uploadService : System.Web.Services.WebService
    {
        ...
        public void UploadFile(string key, string token)
        {
            ...
            //File.WriteAllText(Server.MapPath( ClassCollection.Methods.getfilePath() + "log2.txt"), "hoooy");
            var db = new DataAccessDataContext();
            ...
            //File.WriteAllText(Server.MapPath( ClassCollection.Methods.getfilePath() + "log3.txt"), "hoooy");
            ...
            if (tokenRec.receivedPacket == tokenRec.packetCount)
            {
                ...
                //File.WriteAllText(Server.MapPath( ClassCollection.Methods.getfilePath() + "log.txt4"), "hoooy");
                foreach (var item in t)
                {
                    base64 += item.data;
                }
                var path = ClassCollection.Methods.getfilePath();
                var filename = "appinno" + "_" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + "_" + Guid.NewGuid().ToString("N") + "." + tokenRec.format;

                var log = "log" + "_" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + "_" + Guid.NewGuid().ToString("N") + ".txt";

                //File.WriteAllText(Server.MapPath(path + log), base64);
